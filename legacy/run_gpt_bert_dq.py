...
def main():
    ...
    if args.task_name is not None:
        if args.task_name in ['wnli', 'rte', 'mrpc', 'cola', 'sst2', 'qnli', 'qqp', 'mnli']:
            # metric = evaluate.load("glue", args.task_name, experiment_id=f"{args.output_dir}", cache_dir='./pretrained/evaluate')
            metric = evaluate.load("glue", args.task_name, cache_dir='./pretrained/evaluate')
        elif args.task_name in ['cb', 'wic', 'boolq']:
            metric = evaluate.load("super_glue", args.task_name, experiment_id=f"{args.output_dir}", cache_dir='./pretrained/evaluate')
        else:
            metric = evaluate.load('accuracy', experiment_id=f"{args.output_dir}", cache_dir='./pretrained/evaluate')
    else:
        metric = evaluate.load("accuracy", cache_dir='./pretrained/evaluate')
